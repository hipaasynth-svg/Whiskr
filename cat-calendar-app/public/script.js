...
async function loadStatus() {
  try {
    ...
    if (data.lastWinner && nameEl && photoEl) {
      ...
      if (blurbEl) blurbEl.textContent = `Chosen as Cat of the Month by the last batch's judging. Their calendar — with the rest of that batch's finalists — is in the shop below.`;
    }
  } catch (err) {
  ...
  entryForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      note.textContent = data.groupSealed
        ? "You're in — your batch just sealed and judging starts now. Check your email within 3 weeks."
        : "You're entered! We'll email you once your batch fills (or 3 weeks pass, whichever's first) and judging closes.";
      entryForm.reset();
      loadStatus();
    } catch (err) {
